...
public:
	...
	//   x = ty + sy * y 
	//   x = tz + sz * z 
	void transform_points(float tx, float ty, float tz,
	...
	//       added.
	void add_edge(float x1, float y1, float z1,
	...
	// the ignore function.
	void add_edges(float dmax,
	               const std::function<bool(float,float,float,
	                                        float,float,float)>& ignore);
