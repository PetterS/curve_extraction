...
TEST_CASE("Interpolate/Unary", "")
{
	...
	// Makes sure the length regularization is ok
	auto test_unary = [&data_term]
	(double x, double y, double z, double dx, double dy, double dz, double value) -> void
	{
		double forward_cost = data_term.evaluate_line_integral(x,y,z, x+dx, y+dy, z+dz);
		double backward_cost = data_term.evaluate_line_integral(x+dx, y+dy, z+dz, x,y,z);
	...
	test_unary(2,2,0,	0,2,0, 		34.0);
	test_unary(1,2,3,	0,0,0,		0);
	test_unary(0,0,0,	4,4,4,		4.295486002770816e+02);
	test_unary(1,2,4,	-1,-1,-1,	1.654108574926568e+02);
	test_unary(4,0,2,	-1,4,0,		2.618172076554679e+02);
	test_unary(4,4,2,	0,-4,0,		256);
}
...
void perform_stress_test()
{
	...
	for (int iter = 1; iter <= 100; ++iter) {
		...
		if (d > 1e-6) {
			CHECK((std::abs(real_integral - integral) / (0.5 * (integral + real_integral)) <= 1e-4));
		}
		else {
...
TEST_CASE("Stress test -- PieceWiseConstant")
{
	perform_stress_test<PieceWiseConstant>();
}
...
void perform_differentiation_test()
{
	...
	for (int iter = 1; iter <= 100; ++iter) {
		...
		F f_x1 = x1; f_x1.diff(0);
		F f_y1 = y1; f_y1.diff(1);
		F f_z1 = z1; f_z1.diff(2);
		F f_x2 = x2; f_x2.diff(3);
		F f_y2 = y2; f_y2.diff(4);
		F f_z2 = z2; f_z2.diff(5);
		...
		CHECK((std::abs(dIdx1_numeric - dIdx1_auto) / (0.5 * (dIdx1_numeric + dIdx1_auto)) <= 1e-4));
		CHECK((std::abs(dIdy1_numeric - dIdy1_auto) / (0.5 * (dIdy1_numeric + dIdy1_auto)) <= 1e-4));
		CHECK((std::abs(dIdz1_numeric - dIdz1_auto) / (0.5 * (dIdz1_numeric + dIdz1_auto)) <= 1e-4));
		CHECK((std::abs(dIdx2_numeric - dIdx2_auto) / (0.5 * (dIdx2_numeric + dIdx2_auto)) <= 1e-4));
		CHECK((std::abs(dIdy2_numeric - dIdy2_auto) / (0.5 * (dIdy2_numeric + dIdy2_auto)) <= 1e-4));
		CHECK((std::abs(dIdz2_numeric - dIdz2_auto) / (0.5 * (dIdz2_numeric + dIdz2_auto)) <= 1e-4));
	}
}
