...
TEST_CASE("differentiate_curvature")
{
	...
	Dual x2 = 0.885456025653210; x2.diff(3);
	Dual y2 = 0.464723172043769; y2.diff(4);
	Dual z2 = 0.025641025641026; z2.diff(5);

	Dual x3 = 0.568064746731156; x3.diff(6);
	Dual y3 = 0.822983865893656; y3.diff(7);
	Dual z3 = 0.051282051282051; z3.diff(8);

	auto k = compute_curvature(x1,y1,z1,
	                           x2,y2,z2,
	                           x3,y3,z3);

	double dk_dx1 = k.d(0);
	auto k_x1 = [&] (double x) { return compute_curvature(x,      y1.x(), z1.x(),
	                                                      x2.x(), y2.x(), z2.x(),
	                                                      x3.x(), y3.x(), z3.x()); };
	...
	double dk_dy3 = k.d(7);
	auto k_y3 = [&] (double y) { return compute_curvature(x1.x(), y1.x(), z1.x(),
	                                                      x2.x(), y2.x(), z2.x(),
	                                                      x3.x(), y,      z3.x()); };
...
TEST_CASE("differentiate_torsion")
{
	...
	Dual x4 = 0.120536680255323; x4.diff(9);
	Dual y4 = 0.992708874098054; y4.diff(10);
	Dual z4 = 0.076923076923077; z4.diff(11);

	auto t = compute_torsion(x1, y1, z1,
	                         x2, y2, z2,
	                         x3, y3, z3,
	                         x4, y4, z4);

	double dt_dx1 = t.d(0);
	auto t_x1 = [&] (double x) { return compute_torsion(x,      y1.x(), z1.x(),
	                                                    x2.x(), y2.x(), z2.x(),
	                                                    x3.x(), y3.x(), z3.x(),
	                                                    x4.x(), y4.x(), z4.x()); };
	...
	auto t_y4 = [&] (double y) { return compute_torsion(x1.x(), y1.x(), z1.x(),
	                                                    x2.x(), y2.x(), z2.x(),
	                                                    x3.x(), y3.x(), z3.x(),
	                                                    x4.x(), y,      z4.x()); };
